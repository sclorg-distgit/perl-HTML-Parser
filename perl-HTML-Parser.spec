%{?scl:%scl_package perl-HTML-Parser}
...
BuildRequires:  %{?scl_prefix}perl(HTML::Tagset) >= 3
%if %{undefined perl_bootstrap}
# This creates cycle with perl-HTTP-Message.
BuildRequires:  %{?scl_prefix}perl(HTTP::Headers)
%endif
...
Requires:       %{?scl_prefix}perl(HTML::Tagset) >= 3
Requires:       %{?scl_prefix}perl(HTTP::Headers)
